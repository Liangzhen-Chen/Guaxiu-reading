...
export default function ReadPage() {
  ...
  async function loadState() {
    ...
    if (s === "reading" && !started.current) {
      if (!d.last_messages?.length) { started.current = true; setTimeout(() => sendMsg("开始"), 500); }
    }
  }
  ...
  async function sendMsg(msg?: string) {
    ...
    if (status === "assessment") {
      ...
      if (d.assessment_complete && !started.current) { setStatus("reading"); started.current = true; setTimeout(() => sendMsg("开始"), 600); }
      setStreaming(false);
    } else {
      ...
      while (true) {
        ...
        const clean = full.replace(/\*\*([^*]+)\*\*/g, '$1').replace(/^###+\s?/gm, '').replace(/^---+\s?$/gm, '').replace(/`([^`]+)`/g, '$1');
      ...
      if (isFirst) setChapterTitle(`第 ${chapter} 章`);
  ...
  if (status === "select-mode") {
    return (
      <div className="max-w-lg mx-auto mt-16">
        ...
        {[{id:"quick",t:"快速导读",d:"AI讲解为主，15分钟/章"},{id:"balanced",t:"原文交互",d:"原文与对话交替，30分钟/章"},{id:"deep",t:"深度精读",d:"逐段精读，45分钟/章"}].map(m=>(
          <div key={m.id} onClick={()=>selectMode(m.id)} className={`cursor-pointer rounded-xl p-5 mb-3 transition-all ${mode===m.id?"border-2 border-stone-800 bg-stone-50":"border border-stone-200 bg-white"}`}>
            <span className="font-display font-semibold">{m.t}</span><span className="text-sm ml-2 text-stone-400">{m.d}</span>
          </div>
        ))}
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4 text-sm">
        <div className="flex items-center gap-3">
          <span className="font-display font-bold text-stone-800">{chapterTitle || `第 ${chapter} 章`}</span>
          <span className="text-stone-400">/ {total} 章</span>
          <div className="h-1.5 w-32 rounded-full bg-stone-100"><div className="h-1.5 rounded-full bg-stone-800" style={{width:`${progress}%`}}/></div>
        </div>
        ...
        <div className="w-44 shrink-0 hidden md:block">
          <div className="sticky top-20 space-y-3">
            <div className="rounded-xl border border-stone-200 bg-white p-3">
              <div className="text-xs font-semibold text-stone-400 mb-2 uppercase tracking-wide">{L()==="zh"?"本章引导":"Guide"}</div>
              {concepts.length > 0 ? concepts.map((c,i)=>(
                <div key={i} onClick={()=>{setCurrentConcept(i);sendMsg(L()==="zh"?`我想聊聊「${c}」`:`Tell me about "${c}"`);}}
              ...
              )) : <div className="text-xs text-stone-400">{L()==="zh"?"AI正在分析本章…":"Analyzing chapter…"}</div>}
              <button onClick={()=>sendMsg("/next")} className="mt-2 w-full text-xs py-1 rounded border border-stone-200 text-stone-400 hover:bg-stone-50">{L()==="zh"?"跳过本章 →":"Skip →"}</button>
            </div>
          </div>
        </div>
              ...
              {messages.map((m,i)=>(
                <div key={i} className={`flex ${m.role==="user"?"justify-end":"justify-start"}`}>
                  <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${
                    ...
                    {m.isSummary && <div className="text-xs text-amber-500 mb-1 font-medium">📋 {L()==="zh"?"本章概要":"Summary"}</div>}
                    {m.content}
                  </div>
                </div>
              ))}
              {streaming && messages.length===0 && <p className="text-stone-400 text-sm text-center py-8">{L()==="zh"?"AI 正在准备本章…":"AI preparing…"}</p>}
              <div ref={bottomRef}/>
            </div>
      ...
      {wikiSelection && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20">
          <div className="bg-white rounded-2xl p-6 w-96 max-h-[70vh] overflow-y-auto shadow-xl">
            ...
            <p className="text-xs text-stone-400 mb-4">{L()==="zh"?"勾选要保存的概念":"Check to save"}</p>
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={()=>setWikiSelection(null)} className="cursor-pointer rounded-xl py-2.5 px-4 text-sm text-stone-400 border">{L()==="zh"?"取消":"Cancel"}</button>
            </div>
          </div>
        </div>
      )}
